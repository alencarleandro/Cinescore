...
function setupLikeDislike(id) {
  ...
  likeBtn.addEventListener('click', () => {
    ...
    likeCountEl.textContent = getLikeDislikeCount(id, 'like');
    dislikeCountEl.textContent = getLikeDislikeCount(id, 'dislike');
  });
...
async function carregarFavoritos() {
  ...
  if (localStorage.getItem("emailUsuario") != null) {
    ...
    const nomes = [];
    const quantidades = [];
    const responseTitulosMenosCurtidos = await fetch(`${API_BASE_URL}/api/feedback/titulosMenosCurtidos`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    const titulosMenosCurtidos = await responseTitulosMenosCurtidos.json();

    for (i = 0; i < titulosMenosCurtidos.length; i++) {
      let data = titulosMenosCurtidos[i];

      if (data.tipo === "movie") {
        let xData = await get_movie_by_id(data.id);
        if (xData && xData.poster_path) {
          deslikeGrid.innerHTML += `
    ...
    const ctx = document.getElementById('deslikeChart').getContext('2d');
  ...
  const responsePorcentagem = await fetch(`${API_BASE_URL}/api/feedback/indicadorPorcentagemNegativa`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    document.getElementById('porcentagem').innerHTML = `${porcentagemX}% dos titulos são avaliados negativamente`
