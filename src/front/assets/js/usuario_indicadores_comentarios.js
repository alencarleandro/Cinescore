...
async function carregarFavoritos() {
  ...
  if (localStorage.getItem("emailUsuario") != null) {
    ...
    const responseTitulosMaisComentados = await fetch(`${API_BASE_URL}/api/feedback/titulosMaisComentados`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    const titulosMaisComentados = await responseTitulosMaisComentados.json();

    for (i = 0; i < titulosMaisComentados.length; i++) {
      let data = titulosMaisComentados[i];

      if (data.tipo === "movie") {
        let xData = await get_movie_by_id(data.id);
        if (xData && xData.poster_path) {
          comentGrid.innerHTML += `
    ...
    const responsePorcentagem = await fetch(`${API_BASE_URL}/api/feedback/indicadorComentario`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    document.getElementById('porcentagem').innerHTML = `${porcentagemX}% dos feedbacks registrados são comentados`
