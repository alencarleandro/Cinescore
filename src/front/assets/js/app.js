...
async function setupLikeDislike(id) {
  ...
  const response1 = await fetch(`${API_BASE_URL}/api/feedback/like?tituloId=${id}`, {
    method: 'GET',
    headers: {
  ...
  const response2 = await fetch(`${API_BASE_URL}/api/feedback/deslike?tituloId=${id}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
...
async function setupComments(id) {
  ...
  const response = await fetch(`${API_BASE_URL}/api/feedback/comentarios?tituloId=${id}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
  ...
  commentBtn.addEventListener("click", async () => {
    ...
    await fetch(`${API_BASE_URL}/api/feedback/comentar`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function fetch_favorite_movies(ids) {
  ...
  for (const id of movie_ids) {
    ...
    await fetch(`${API_BASE_URL}/api/generosUsuario/salvar?generos=${generos}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function fetch_trending() {
  ...
  // Esconde a seção de tendências se não houver cards
  const trendingSection = document.querySelector('.trending');
  if (trendingSection) {
    trendingSection.style.display = trending_el.children.length > 0 ? 'block' : 'none';
  }
}
...
async function gerenciarFavorito(id, tipo) {
  if (localStorage.getItem("emailUsuario") != null) {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/titulosfavoritos/gerenciar?email=${localStorage.getItem("emailUsuario")}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
async function carregarFavoritos() {
  ...
  const response1 = await fetch(`${API_BASE_URL}/api/titulosfavoritos/listaFilmes?email=${localStorage.getItem("emailUsuario")}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
  ...
  const response2 = await fetch(`${API_BASE_URL}/api/titulosfavoritos/listaSeries?email=${localStorage.getItem("emailUsuario")}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
  ...
  // Esconde ou mostra as seções de acordo com a existência de cards
  const favoritesSection = document.querySelector('.favorites');
  if (favoritesSection) {
    const favoritesGrid = favoritesSection.querySelector('.movies-grid');
    favoritesSection.style.display = favoritesGrid && favoritesGrid.children.length > 0 ? 'block' : 'none';
  }

  const recomendacaoSection = document.querySelector('.recomendacao');
  if (recomendacaoSection) {
    const recomendacaoGrid = recomendacaoSection.querySelector('.movies-grid');
    recomendacaoSection.style.display = recomendacaoGrid && recomendacaoGrid.children.length > 0 ? 'block' : 'none';
  }
}

async function generoPreferido() {
  const response1 = await fetch(`${API_BASE_URL}/api/generosUsuario/generoFavorito?email=${localStorage.getItem("emailUsuario")}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json'
    }
...
async function limparGeneros() {
  ...
  await fetch(`${API_BASE_URL}/api/generosUsuario/limpar`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json'
    },
