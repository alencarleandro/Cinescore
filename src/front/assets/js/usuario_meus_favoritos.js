...
async function fetch_user_favorite_movies(ids) {
  const x_ids = ids;
...
async function gerenciarFavorito(id) {
  ...
  if (localStorage.getItem("emailUsuario") != null) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/titulosfavoritos/gerenciar`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
async function carregarFavoritos() {
  ...
  if (localStorage.getItem("emailUsuario") != null) {
    ...
    fetch_user_favorite_movies(titulos1);
    ...
    fetch_user_favorite_series(titulos2)
...
async function iniciarCampos() {
  ...
  if (localStorage.getItem("emailUsuario") != "admin@cinescore.com") {
    document.querySelector('#indicadoresbom').style.display = 'none';
    document.querySelector('#indicadoresmal').style.display = 'none';
    document.querySelector('#indicadorescom').style.display = 'none';

    
    const response1 = await fetch(`${API_BASE_URL}/api/usuarios/nome?email=${localStorage.getItem("emailUsuario")}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    const response2 = await fetch(`${API_BASE_URL}/api/usuarios/senha?email=${localStorage.getItem("emailUsuario")}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
...
document.getElementById('remover').addEventListener('click', async function () {
  const action = await fetch(`${API_BASE_URL}/api/usuarios/deletar?email=${localStorage.getItem("emailUsuario")}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json'
    }
...
document.getElementById('btnSalvar').addEventListener('click', async function () {
  ...
  const action = await fetch(`${API_BASE_URL}/api/usuarios/alterar?email=${localStorage.getItem("emailUsuario")}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
    },
